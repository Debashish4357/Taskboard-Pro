...
const Dashboard = () => {
  ...
  if (isDashboardLoading || isTeamLoading) {
    return (
      <div className='py-10'>
        <Loading />
      </div>
  ...
  const totals = dashboardData?.tasks || { todo: 0, "in progress": 0, completed: 0 };
    ...
    return (
      <div className='w-full h-32 bg-white p-5 shadow-md rounded-md flex items-center justify-between'>
        <div className='h-full flex flex-1 flex-col justify-between'>
          ...
          <span className='text-sm text-gray-400'>{"Last updated now"}</span>
        </div>
  ...
  return (
    <div className='h-full py-4'>
      ...
      <div className='w-full flex flex-col md:flex-row gap-4 2xl:gap-10 py-8'>
        ...
        {/* /right */}
        <UserTable users={teamData?.users || []} />
      </div>
    </div>
  );
};
